...
public:
  ...
  CShow_RefIndices(TComPic* pcPic, int NumCTBs, char *VizFile, int CTBSize,
      bool isMarkingBlocks = true, bool isDrawingVectors = false )
  {
    //---------------------------------------------
    // CHANGE HERE
    //---------------------------------------------
    ...
    call = 0;
    sprintf(text, "====%s(%d)====", "CShow_RefIndices", call);
...
//      markCTB (pcPic);
    ...
    // RENDERING TO JPG
    //---------------------------------------------
	sprintf(text, "./jpg/RefIndices_Poc%04d.jpg", pcPic->getPOC());
	sprintf(VizFile, "RefIndices_Poc%04d.jpg", pcPic->getPOC()); //so that caller knows name of file
